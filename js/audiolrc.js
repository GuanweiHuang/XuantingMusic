...
function playlrc(jump){
    ...
    if(jump == 1){
        ...
        //重新计算当前位置歌词的索引
        num = 0;
        while(audio.currentTime >= (lrctime[num+1]+setDelayTime-0.1)) {num++;}
        if(num == 0){
    ...
    //如果不是跳转操作，则让歌词开始从头播放
        ...
        setDelayTime = parseFloat(localStorage.getItem(musicname[index])) || 0;
    ...
    playing = setInterval(function(){
        ...
        //如果歌曲时间存在，按时间遍历歌词
        if(audio.currentTime && audio.currentTime >= (lrctime[num]+setDelayTime-0.1)){
            ...
            if(num != 0){
                ...
                document.getElementById("li"+(num-1)).style.color = "#DDD";
            }
...
function delrc(denum){
    if(songlrc){
        ...
        var index = window.parent[0].num;
        var musicname = window.parent[0].musicname;
        localStorage.setItem(musicname[index],Math.round(setDelayTime*10)/10);
    }
}